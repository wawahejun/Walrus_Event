  ...
  return (
    <div className="h-full w-full p-4 md:p-8 flex flex-col gap-8 text-gray-800 pb-24 overflow-y-auto no-scrollbar">
       <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#F59E0B] to-white">REPUTATION SYSTEM</h1>
          <p className="text-sm text-gray-600 mt-1">Sovereign Identity & Trust Score</p>
        </div>
      ...
      <div className="flex-1 min-h-[400px] flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-2/3 bg-white/70 rounded-2xl border border-amber-200 p-8 relative overflow-hidden">
           <h3 className="text-sm font-bold text-gray-600 absolute top-6 left-6 uppercase tracking-wider">Reputation State Machine</h3>
...
const StateNode = ({ x, y, label, active }: any) => (
  ...
  >
    ...
    <span className={cn("text-xs font-medium", active ? "text-gray-800" : "text-gray-600")}>{label}</span>
  </div>
)
